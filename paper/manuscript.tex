...
\begin{enumerate}
	...
	\item MANOVA test: how do the (means of the) classes vary in the joint space?
\end{enumerate}
...
I think the answer to this is that EFDs are fundamentally different from pixel locations, since pixel locations are all
equally important but an EFD series can be truncated and still keep most of the important information.
This difference might mean that PCA is a more natural algorithm to use on EFDs than on real-space coordinates, but
that might not be true.
Either way, finding biologically inspired features and deciding how best to express them,
either in terms of real-space pixel co-ordinates or a conjugate space like EFDs,
is probably the best way to select features for shape analysis.
